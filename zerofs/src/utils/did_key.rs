...
pub type Bls12381KeyPair = KeyPair<did_key::Bls12381KeyPairs>;
...
impl JwsAlgName for did_key::Bls12381KeyPairs {
    fn get_jws_algorithm_name() -> String {
        "BBS+".to_string()
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bls12381_key_pair() {
        let key = Bls12381KeyPair::generate(None);
        let did = key.get_did().await.unwrap();
